...
func GetSeparator(l string) string {
	id := GetLocaleId(l)
	_MUTEX_STEP.RLock()
	defer _MUTEX_STEP.RUnlock()
	sep := _LANG_SEP[id]
	if sep == "" {
		sep = ","
	}
...
var transAfter = func(c Code, lanId int, value string) string {
	return value
}

func RegisterTransAfter(fu func(c Code, lanId int, value string) string) {
	transAfter = fu
}
...
func (c Code) transOne(id int) string {
	...
	return transAfter(c, id, entries[id][c-_start-1])
}
