...
func build_code_go(langs, keySort []string, kvMap map[string]map[string]string) {
	...
	var enumBuilder1, enumBuilder2 strings.Builder
...
const _NUM_LANG uint16 = `+strconv.Itoa(len(langs))+` // 不包含key
...
const (`))
	...
	for _, locale := range langs {
		...
		enumBuilder1.WriteString(fmt.Sprintf("\n	%s,", varName))
	}
	write(enumf, `
)
`)
	write(enumf, "var _default_langsort = [_NUM_LANG]string{")
	write(enumf, enumBuilder1.String())
	write(enumf, `
}
`)
	write(enumf, `
const (
	...
	write(enumf, enumBuilder2.String())
	write(enumf, `
	_end
)
`)
